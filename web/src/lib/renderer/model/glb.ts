...
import { loadGLTF, loadStaticGLTF } from "$lib/renderer/model/gltf";
...
export async function loadStaticGLB(gl: WebGL2RenderingContext, blob: Blob) {
  const buffer = await blob.arrayBuffer();
  const view = new DataView(buffer);
  ...
  // glb is just a glTF with a preloaded buffer
  return await loadStaticGLTF(gl, gltf, [chunk1Data]);
}
